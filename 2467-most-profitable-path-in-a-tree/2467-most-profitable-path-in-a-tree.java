class Solution {
    ...
    public int findAlicePath(int start, int time, HashMap<Integer, HashSet<Integer>> tree, HashMap<Integer, Integer> bobTime, HashSet<Integer> visited, int[] amount){
    ...
    public int findBobPath(int start, int bob, HashMap<Integer, HashSet<Integer>> tree, HashMap<Integer, Integer> bobTime, HashSet<Integer> visited){
        // Bob start position
        if(start == bob){
            bobTime.put(start, 0);
            return 0;
        }
        // Leave node and not bob
        if(tree.get(start).size() == 1 && visited.contains(tree.get(start).iterator().next())){
            return -1;
        }
        int time = -1;
